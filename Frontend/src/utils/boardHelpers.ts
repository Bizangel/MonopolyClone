...
export function imagepixels_to_tileindex(x: number, y: number) {
  var hallway_idx = -1;
  var detectvar = -1;
  if (y < bc.cornerLengthImg) {
    hallway_idx = 3; // third one up
    detectvar = x;
  } else if (y > bc.boardImgSize - bc.cornerLengthImg) {
    hallway_idx = 1; // first one up
    detectvar = bc.boardImgSize - x;
  } else if (x < bc.cornerLengthImg) {
    hallway_idx = 2;
    detectvar = bc.boardImgSize - y;
  } else if (x > bc.boardImgSize - bc.cornerLengthImg) {
    hallway_idx = 4;
    detectvar = y;
  }
  ...
  if (detectvar < bc.cornerLengthImg) {
    detected = 0;
  } else if (detectvar > bc.boardImgSize - bc.cornerLengthImg) {
    detected = 10
  } else {
    for (detected = 0; detected < 9; detected++) {
      var helpLoc = detectvar - bc.cornerLengthImg; // location removing corner
      if (
        (detected * bc.tileLengthImg < helpLoc) && (
          helpLoc < (detected + 1) * bc.tileLengthImg)) {
        detected += 1;
        break
      }
    }
...
function pixelToBoardXZ(px: number) {
  return px / bc.boardImgSize * bc.boardSize - bc.boardSize / 2
}

const board_bottom = bc.boardSize / 2;
const board_top = - bc.boardSize / 2;

const board_left = - bc.boardSize / 2;
const board_right = bc.boardSize / 2;

const firstrow_topy = pixelToBoardXZ(bc.boardImgSize - bc.cornerLengthImg);
const secondrow_rightx = pixelToBoardXZ(bc.cornerLengthImg);
const thirdrow_bottomy = pixelToBoardXZ(bc.cornerLengthImg);
const fourthrow_leftx = pixelToBoardXZ(bc.boardImgSize - bc.cornerLengthImg);

const cornerBoardLength = bc.cornerLengthImg / bc.boardImgSize * bc.boardSize;
const tileBoardLength = bc.tileLengthImg / bc.boardImgSize * bc.boardSize;

function XZVector(x: number, z: number) {
  return new Vector3(x, bc.boardYLocation, z);
}
