...
import { getInwardDirection, housebarHeight, playerSeparationDistance, tileToWorldLocation } from "../../../common/boardhelpers"
...
export function PlayerModelHandler(props: PlayerLocationHandlerProps) {
  ...
  props.locations.forEach((location, character) => {
    ...
    if (stackedPlayers !== undefined && stackedPlayers > 1) {
      ...
      topmidpoint.add(sepDir.clone().multiplyScalar(housebarHeight)); // offset by housebar

      topmidpoint.add(sepDir.clone().multiplyScalar(playerSeparationDistance * (resolvedPosPlayers))); // offset according to players
